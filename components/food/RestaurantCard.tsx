...
import { Search, Clock, ThumbsUp, ChevronDown, Star, Menu } from "lucide-react"
import { Card, CardContent } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import Link from "next/link"
...
const FoodDeliveryApp = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden mb-6">
          <Button variant="ghost" className="w-full justify-start gap-2 mb-2">
            <Clock className="h-4 w-4" />
            Delivering to: Alhaji Bashiru Street, 11
          </Button>
          <Button asChild variant="ghost" className="w-full justify-start gap-2 mb-2">
            <Link href="/NearMe">
              <Clock className="h-4 w-4" />
              NearMe
            </Link>
          ...
          {filters.map((filter) => (
            <Button key={filter.name} variant="ghost" className="w-full justify-start gap-2 mb-2">
              <span>{filter.icon}</span>
              {filter.name}
            </Button>
          ))}
      ...
      <div className="relative mb-8">
        <Search className="absolute left-3 top-3 h-5 w-5 text-gray-400" />
        <Input placeholder="What can we get you?" className="pl-10 py-6 text-lg" />
      </div>
        ...
        <div className="w-full md:w-64 mb-6 md:mb-0">
          <div className="hidden md:block">
            ...
            <div>
              <h3 className="font-medium mb-3">Popular filters</h3>
              {filters.map((filter) => (
                <Button key={filter.name} variant="ghost" className="w-full justify-start gap-2 mb-1">
                  <span>{filter.icon}</span>
                  {filter.name}
                </Button>
              ))}
